...
@Slf4j
public class RocketMQTemplate extends AbstractMessageSendingTemplate<String> implements DisposableBean {
    ...
    public SendResult syncSend(String destination, Message<?> message, long timeout) {
        if (isNotRunning()) {
            log.info("rocket mq not running");
    ...
    private boolean isRunning() {
        return producer.getDefaultMQProducerImpl().getServiceState() == RUNNING;
    }
}
